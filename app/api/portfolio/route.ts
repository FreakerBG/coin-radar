...
export async function GET(){const user=await getChatGPTUser();if(!user)return Response.json({error:'Sign in to load your research account.'},{status:401});try{const database=db();const [config,positions,events]=await Promise.all([getConfig(user.userId),getPositions(user.userId),database.prepare('SELECT data FROM research_events WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').bind(user.userId).all<{data:string}>()]);return Response.json({config,positions,events:events.results.map(r=>JSON.parse(r.data)),monitoring:'browser_open_only'},{headers:{'Cache-Control':'no-store'}});}catch{return Response.json({error:'Research storage unavailable. Your saved positions have not been changed.'},{status:503});}}
export async function POST(request:Request){const user=await getChatGPTUser();if(!user)return Response.json({error:'Sign in required.'},{status:401});if(!sameOrigin(request))return Response.json({error:'Same-origin request required.'},{status:403});try{
 const b:any=await request.json();if(b.action==='config'){const config=configSchema.parse(b.config);await db().prepare('INSERT INTO research_accounts (user_id, config) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET config = excluded.config, revision = research_accounts.revision + 1').bind(user.userId,JSON.stringify(config)).run();return Response.json({ok:true});}
 ...
 }catch(e){return Response.json({error:e instanceof z.ZodError?'Check the amounts and alert percentages.':'Could not save. Your input is still available; try again.'},{status:e instanceof z.ZodError?400:503});}}
