...
// Everything is local: no remote Wrangler or D1 command, no account, and Wrangler's banner (and with
// it its npm update check) is off. Only the process tree and the temporary directory created here are
// ever stopped or removed.
...
// The captured log keeps only its most recent part; watched texts are counted over all of it.
const LOG_TAIL_CHARS = 1024 * 1024;
...
function startArguments(stateDir, port, vars) {
  const script = JSON.parse(readFileSync(path.join(projectRoot, 'package.json'), 'utf8')).scripts.start;
  // Split on whitespace, so the script must not quote or escape arguments; anything unexpected stops the test.
  const [command, ...args] = script.trim().split(/\s+/);
  const persist = args.indexOf('--persist-to');
  if (/["'`\\]/.test(script) || command !== 'node' || persist < 0 || !args.includes('--local') || args.includes('--remote')) {
    throw new Error('Unexpected npm start script: ' + script);
  }
...
// `watch` lists texts to count in the Worker's output (see worker.count).
export async function startBuiltWorker({vars = {}, watch = []} = {}) {
  ...
  let log = '';
  let carry = '';
  const counts = new Map(watch.map(text => [text, 0]));
  const longest = Math.max(1, ...watch.map(text => text.length));
  function capture(chunk) {
    log = (log + chunk).slice(-LOG_TAIL_CHARS);
    // Carry the end of the previous chunk so a watched text split across two chunks is still counted.
    const text = carry + chunk;
    for (const [watched, count] of counts) {
      counts.set(watched, count + text.split(watched).length - carry.split(watched).length);
    }
  ...
  async function waitUntilReady() {
    for (let started = Date.now(); ; await sleep(250)) {
      if (!worker.alive) return 'The Worker exited during startup.';
      if (Date.now() - started > 120000) return 'The Worker did not become ready.';
      try {
        if ((await fetch(worker.origin + '/api/health', {signal: AbortSignal.timeout(2000)})).status === 401) return true;
      } catch {
        // Not listening yet.
      }
    }
  ...
  const worker = {
    ...
    get log() { return log; },
    count(text) {
      if (!counts.has(text)) throw new Error(`${text} is not watched`);
    ...
    // they cannot leak. Removes the temporary state unless `keepTemp`.
    async stop({keepTemp = false} = {}) {
      let survivors = [];
      if (child) {
      ...
      if (keepTemp) return {survivors, tempRemoved: false};
      try {
  ...
  try {
    ...
    // The free port can be taken between choosing and binding it; then start again on another one.
    for (let attempt = 1; ; attempt++) {
      ...
      for (const output of [child.stdout, child.stderr]) {
        ...
        output.on('data', capture);
      }
      const ready = await waitUntilReady();
      if (ready === true) break;
      const portTaken = /EADDRINUSE|address already in use/i.test(log);
      const {survivors} = await worker.stop({keepTemp: true});
      if (survivors.length) throw new Error('A Worker process survived a failed start: ' + survivors.join(', '));
      if (!portTaken || attempt === 3) throw new Error(ready + '\n' + log.slice(-4000));
    }
    known = treeProcesses(child.pid, known);
    return worker;
  } catch (error) {
