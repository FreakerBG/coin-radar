...
| Provider or API outage unrelated to schema (DEX Screener, CoinDesk, X) | Do not roll back or migrate. Confirm `/api/health` and `/api/portfolio` return 200 while provider routes return their unavailable messages, monitoring reports `provider_unavailable`, and failure records are `warn` level. | Not needed. | No. | The provider recovers and section 6 passes. |
