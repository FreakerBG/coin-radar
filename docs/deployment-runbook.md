...
| R2 | None (`"r2": null`). |
| Secrets | `X_BEARER_TOKEN`, set in Sites environment settings. A publish applies the environment revision. Never commit or print it. `GOLDMINE_CRON_SECRET` (Stage 03C, optional): authorizes `POST /api/goldmine/scheduled` for an automated caller instead of a ChatGPT session. Unset means that route always answers 401 (fail closed) — leave it unset unless an actual automated caller is wired and confirmed reachable (docs/goldmine-intelligence.md section 6b.1). Never commit or print it. |
...
- **No confirmed scheduler:** whether Sites' publish tooling supports Cloudflare Cron Triggers (a `scheduled` Worker export) is unverified, and there is no repository-visible way to declare one — `.openai/hosting.json` has no field for it. `POST /api/goldmine/scheduled` (Stage 03C, docs/goldmine-intelligence.md section 6b) is a real, tested, fail-closed automated entry point, but nothing currently calls it on a schedule in production. The project's separate Vercel deployment cannot run it at all: that build has no D1 binding by design.
