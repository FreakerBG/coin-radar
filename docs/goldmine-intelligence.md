...
`POST /api/goldmine` (signed in, same origin, one shared lock `goldmine:scan`) first settles due outcomes, then discovers, scores and records. `GET /api/goldmine` reads recent signals and statistics. Without the D1 binding (the Vercel preview) both answer 503 before any provider call.
...
- `contractSafety` is always `{status: 'unavailable'}`: no configured provider supplies it. `{status: 'verified', source}` is the shape a future provider must fill.
- The snapshot is stored with every signal, so a later model version can re-score exactly what was observed.

## 3. Momentum Score v2 (`momentum-v2.0.0`)

Deterministic: the assessment depends only on the snapshot, never on the clock. Any change to a rule or threshold must change `MODEL_VERSION`.
...
| Token age and valuation | 15 | Age <1h 2, <6h 5, <72h 8, <30d 6, older 3. Valuation (market cap, else FDV) <$100k 2, <$1M 7, <$10M 5, <$50M 3, larger 1; none if FDV exceeds 5x market cap. |
...
| Safety risk | 15 | Starts at 8 while contract safety is unavailable (15 once verified). Minus 3 each: >50% 1h rise, >30% 24h fall, turnover >30x, buy share >80%. Minus 2 each: FDV >5x market cap, valuation >100x liquidity, pool younger than 1h. Never below 0. |

Every component lists its evidence. A component whose inputs are missing scores nothing, and the tests check that removing any input never raises the score.
...
A candidate is an opportunity only if it is `EARLY`, `BUILDING` or `BREAKOUT`, scores at least 60, and has verified contract safety. Each unmet condition is a listed blocker. **Because no provider supplies contract safety, no candidate is an opportunity in Stage 03A.** This is intentional fail-closed behavior, not a defect.
...
- **Contract safety provider:** fill `contractSafety` in the snapshot. The score and the opportunity gate already handle `verified`.
...
| 03B | Contract safety evidence (mint and freeze authority, top-holder concentration, LP status), so opportunities can exist; Goldmine dashboard panel with explanations and outcome history. | Safety provider (for example RugCheck's public API) |
...
- DEX Screener provides no contract safety, unique-wallet counts or holder data. Buyer growth uses swap counts.
